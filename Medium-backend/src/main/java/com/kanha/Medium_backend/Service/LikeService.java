...
public class LikeService {
    ...
    public void addLike(UUID articleId) {
        ...
        UUID currUserOpeartingTheWebsId = authServiceImpl.providingUserId();
        //currArticle's user id
        UUID userIdOfThisArticle = articleRepo.findById(articleId).orElseThrow(() -> new RuntimeException("Not found")).getCustomUser().getId();
        if (!userIdOfThisArticle.equals(currUserOpeartingTheWebsId)) { // if both users are different then only like perform
            ...
            like.setCustomUser(userRepo.findById(currUserOpeartingTheWebsId).orElseThrow(() -> new RuntimeException("Invalid User")));
    ...
    public void deleteLike(UUID articleId) {
        UUID currUserOperatingTheSite = authServiceImpl.providingUserId();

        Like like = likeRepo.findByArticleIdAndCustomUserId(articleId, currUserOperatingTheSite);
        if (like == null)
